...
import { CliArgumentError, executeCli, parseCliArgs } from "./cli.js";
...
describe("parseCliArgs", () => {
	...
	test("throws CliArgumentError for an unknown command", () => {
		assert.throws(
			() => parseCliArgs(["explode", "scenario.yaml"]),
			CliArgumentError,
		);
	});
	...
	test("throws CliArgumentError for an unrecognized flag", () => {
		assert.throws(
			() => parseCliArgs(["run", "scenario.yaml", "--bogus"]),
			CliArgumentError,
		);
	});
});

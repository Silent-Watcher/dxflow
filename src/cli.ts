...
import { renderConsoleReport, renderStepLine } from "./report/console-reporter.js";
...
export function parseCliArgs(argv: string[]): CliOptions {
  ...
  if (command !== "run") {
    throw new CliArgumentError(`Unknown command "${command ?? ""}". Usage: dxflow run <manifest> [options]`);
  }
  ...
  if (!manifestPath) {
    throw new CliArgumentError("Missing required <manifest> argument. Usage: dxflow run <manifest> [options]");
  }
...
async function main(): Promise<void> {
  try {
    ...
    if (error instanceof CliArgumentError || error instanceof ManifestLoadError) {

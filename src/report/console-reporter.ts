 ...
 */
export function renderStepLine(result: StepResult, index: number, total: number): string {
...
export function renderConsoleReport(report: ScenarioReport): string {
  ...
  for (const [index, step] of report.steps.entries()) {
    ...
    for (const expectation of step.expectations.filter((expectation) => !expectation.passed)) {
      lines.push(`      ${SYMBOLS.fail} ${expectation.description}${expectation.details ? ` (${expectation.details})` : ""}`);
    }
  }
  ...
  lines.push(`  Avg duration:   ${report.summary.averageDurationMs.toFixed(1)}ms`);
  if (report.summary.slowestStep) {
    lines.push(`  Slowest step:   ${report.summary.slowestStep.id} (${report.summary.slowestStep.durationMs}ms)`);
  }
  if (report.summary.fastestStep) {
    lines.push(`  Fastest step:   ${report.summary.fastestStep.id} (${report.summary.fastestStep.durationMs}ms)`);
  }

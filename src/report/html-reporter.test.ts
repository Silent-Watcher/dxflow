...
describe("renderHtmlReport", () => {
  ...
  test("escapes HTML special characters in scenario name and URLs", () => {
    const report = makeReport({ scenarioName: '<script>alert("xss")</script>' });
  ...
  test("escapes HTML special characters in step error messages", () => {
    const report = makeReport({
      steps: [makeStepResult({ success: false, error: '<img src=x onerror="alert(1)">' })],
    });
    const html = renderHtmlReport(report);
  ...
  test("renders one table row per step with method, url, status, duration", () => {
    const report = makeReport({
      steps: [makeStepResult({ id: "a" }), makeStepResult({ id: "b", success: false, error: "boom" })],
    });
    const html = renderHtmlReport(report);
  ...
  test("colors failing-step bars differently from passing-step bars", () => {
    const report = makeReport({
      steps: [makeStepResult({ id: "ok", success: true }), makeStepResult({ id: "bad", success: false })],
    });
    const html = renderHtmlReport(report);

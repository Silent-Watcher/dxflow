...
import { sendHttpRequest, HttpTimeoutError } from "./http-client.js";
...
describe("sendHttpRequest", () => {
  ...
  test("parses a JSON response body and reports isJson true", async () => {
    globalThis.fetch = mock.fn(async () => mockJsonResponse(200, { id: 42, name: "Ada" })) as unknown as typeof fetch;
  ...
  test("falls back to raw text when the body is not valid JSON", async () => {
    globalThis.fetch = mock.fn(
      async () => new Response("plain text response", { status: 200, headers: { "content-type": "text/plain" } }),
  ...
  test("measures duration as non-negative and endedAt >= startedAt", async () => {
    globalThis.fetch = mock.fn(async () => mockJsonResponse(200, {})) as unknown as typeof fetch;
  ...
  test("captures response headers", async () => {
    globalThis.fetch = mock.fn(
      async () => mockJsonResponse(200, {}, { "x-request-id": "abc-123" }),
    ) as unknown as typeof fetch;

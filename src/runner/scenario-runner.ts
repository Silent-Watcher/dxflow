...
export interface ScenarioRunOptions {
  ...
  onStepComplete?: (result: StepResult, stepIndex: number, totalSteps: number) => void;
...
export async function runScenario(
  ...
  let context = createInitialContext(manifest.name, manifest.baseUrl, manifest.vars);
  ...
  return buildReport(manifest, options.manifestPath, results, startedAt, endedAt);
}
...
function buildReport(
  ...
  const slowestStep = findExtremeStep(steps, (a, b) => a.timing.durationMs > b.timing.durationMs);
  const fastestStep = findExtremeStep(steps, (a, b) => a.timing.durationMs < b.timing.durationMs);
  ...
  return {
    ...
    summary: {
      ...
      failedSteps,
      averageDurationMs,
      slowestStep: slowestStep ? { id: slowestStep.id, durationMs: slowestStep.timing.durationMs } : undefined,
      fastestStep: fastestStep ? { id: fastestStep.id, durationMs: fastestStep.timing.durationMs } : undefined,
    },
  };
}
...
function findExtremeStep(
  ...
  return steps.reduce((best, current) => (isMoreExtreme(current, best) ? current : best));
}

...
describe("runStep", () => {
  ...
  test("sends a request to baseUrl + path and records a successful context entry", async () => {
    globalThis.fetch = mock.fn(
      async () => new Response(JSON.stringify({ id: 7 }), { status: 201, headers: { "content-type": "application/json" } }),
    ) as unknown as typeof fetch;

    const context = createInitialContext("test scenario", "https://api.example.com", undefined);
    const outcome = await runStep(baseStep(), context, "https://api.example.com", undefined, undefined);

    assert.equal(outcome.result.success, true);
    assert.equal(outcome.result.response?.status, 201);
    assert.equal(outcome.contextEntry?.request.url, "https://api.example.com/users");
  ...
  test("resolves templated headers from prior step context", async () => {
    ...
    let context = createInitialContext("test", "https://api.example.com", undefined);
    context = {
      ...context,
      steps: {
        login: {
    ...
    const step = baseStep({ headers: { Authorization: "Bearer {{steps.login.body.token}}" } });
    await runStep(step, context, "https://api.example.com", undefined, undefined);
  ...
  test("merges defaultHeaders with step-level headers, step-level taking precedence", async () => {
    ...
    const context = createInitialContext("test", "https://api.example.com", undefined);
  ...
  test("applies a transform override on top of resolved values", async () => {
    ...
    await runStep(step, context, "https://api.example.com", undefined, loadStepTransform);

    assert.equal(loadStepTransform.mock.calls.length, 1);
    assert.equal(loadStepTransform.mock.calls[0]?.arguments[0], "./fake.ts#fn");
    assert.equal(capturedUrl?.toString(), "https://api.example.com/users/override");
    assert.equal((capturedInit?.headers as Record<string, string>)["X-From-Transform"], "yes");
    assert.deepEqual(JSON.parse(capturedInit?.body as string), { fromTransform: true });
  });

  test("fails the step when a transform is declared but no loader is provided", async () => {
    const context = createInitialContext("test", "https://api.example.com", undefined);
    const step = baseStep({ transform: "./fake.ts#fn" });
    const outcome = await runStep(step, context, "https://api.example.com", undefined, undefined);
  ...
  test("marks the step failed when an expectation does not pass", async () => {
    globalThis.fetch = mock.fn(
      async () => new Response(JSON.stringify({}), { status: 404, headers: { "content-type": "application/json" } }),
    ) as unknown as typeof fetch;
  ...
  test("fails gracefully when a template expression cannot be resolved", async () => {
    const context = createInitialContext("test", "https://api.example.com", undefined);
    const step = baseStep({ headers: { Authorization: "Bearer {{steps.missing.body.token}}" } });
    const outcome = await runStep(step, context, "https://api.example.com", undefined, undefined);
  ...
  test("resolves query params against context", async () => {
    ...
    const step = baseStep({ method: "GET", path: "/orders", query: { userId: "{{steps.createUser.body.id}}" } });
    await runStep(step, context, "https://api.example.com", undefined, undefined);

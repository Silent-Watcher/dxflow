...
describe("buildUrl", () => {
  test("joins a base URL and a relative path with a leading slash", () => {
    assert.equal(buildUrl("https://api.example.com", "/users", undefined), "https://api.example.com/users");
  });
  ...
  test("uses an absolute path as-is, ignoring baseUrl", () => {
    assert.equal(
      buildUrl("https://api.example.com", "https://other-host.com/ping", undefined),
  ...
  test("appends query params", () => {
    const url = buildUrl("https://api.example.com", "/users", { page: "2", limit: "10" });
  ...
  test("encodes special characters in query values", () => {
    const url = buildUrl("https://api.example.com", "/search", { q: "hello world & friends" });
    const parsed = new URL(url);

...
describe("parseTransformRef", () => {
	test("splits a valid reference into file path and export name", () => {
		...
		assert.deepEqual(result, {
			filePath: "./transforms/foo.ts",
			exportName: "buildBody",
		});
	});
	...
	test("throws when there is no '#' separator", () => {
		assert.throws(
			() => parseTransformRef("./transforms/foo.ts"),
			TransformLoadError,
		);
	});
...
describe("loadTransform", () => {
	...
	test("loads a transform function from a relative path", async () => {
		...
		assert.deepEqual(result, {
			body: { fromTransform: true, scenario: "test" },
		});
	});
	...
	test("supports absolute file paths", async () => {
		...
		const fn = await loadTransform(
			`${absolutePath}#buildBody`,
			"/irrelevant/dir",
		);
		const result = await fn({
			scenarioName: "abs-test",
			baseUrl: undefined,
			steps: {},
			vars: {},
		});
		assert.deepEqual(result, {
			body: { fromTransform: true, scenario: "abs-test" },
		});
	});
});

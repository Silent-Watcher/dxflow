 ...
 */
export function parseTransformRef(ref: string): { filePath: string; exportName: string } {
 ...
 */
export async function loadTransform(ref: string, manifestDir: string): Promise<TransformFn> {
  const { filePath, exportName } = parseTransformRef(ref);

  const absolutePath = isAbsolute(filePath) ? filePath : resolvePath(manifestDir, filePath);
